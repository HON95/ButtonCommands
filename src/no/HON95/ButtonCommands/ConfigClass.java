...
public final class ConfigClass {
	...
	void initConfig() {
		...
		config.addDefault("current.hasPerm", true);
		config.addDefault("current.missingPerm", false);
		config.addDefault("ignoreWhiteLists", true);
	...
	void initAlias() {
		...
		if (!aliasFile.exists()) {
			...
			al.add("say");
			alias.set("speak.enable", false);
			alias.set("speak.commands", al);
			alias.set("speak.current.hasPerm", true);
			alias.set("speak.current.missingPerm", false);
			alias.set("speak.permission", "");
			alias.set("speak.ignoreOtherPerms", false);
			alias.set("speak.ignoreWhiteList", false);
			alias.set("speak.cooldown", 0);
			...
			while (ks.hasNext()) {
				...
				if (!cs.isList("commands")) {
					cs.set("commands", new ArrayList<String>());
					save = true;
				}
