...
public final class PlayerInteractListener implements Listener {
	...
	private boolean signFinder(Player player, Block block, BlockFace[] testBlocks, BlockFace[] testFaces, final Block bLever) {
		...
		for (int c = 0; c < 5; c++) {
			...
			if (s > 0) {
				...
				if (bLever.getType() != Material.LEVER)
					return n;
				Lever lever = (Lever) bLever.getState().getData();
				final boolean powered = lever.isPowered();
				lever.setPowered(!powered);
				bLever.setData(lever.getData());
				bLever.getWorld().playEffect(bLever.getLocation(), Effect.SMOKE, 4);
	...
	private int signConverter(Player player, Block block) {
		...
		return executor(player, block, cmd, false, ignoreWhiteLists, false);
	}

	private int executor(Player player, Block block, String cmd[], boolean ignorePerms, boolean ignoreWhiteList, boolean alias) {

		if (cmd[1].length() == 0) {
			player.sendMessage(RE + "Invalid command sign! Needs to contain a command.");
			return 1;
		}
			...
			return consoleExecutor(player, cmd[1], ignorePerms, ignoreWhiteList);
		}
		else if (cmd[0].equalsIgnoreCase("c")) {
			return consoleExecutor(player, cmd[1], ignorePerms, ignoreWhiteList);
		}
		else if (cmd[0].equalsIgnoreCase("alias") && !alias) {
			return aliasExecutor(player, block, cmd[1], ignorePerms);
		}
			...
			if (ignorePerms || player.hasPermission("buttoncommands.use.normal")) {
				player.chat("/" + (cmd[0].length() > 0 ? cmd[0] + " " : "") + cmd[1]);
				return (curPerm ? 2 : 1);
			}
			else {
	...
	private int consoleExecutor(Player player, String command, boolean ignorePerms, boolean ignoreWhiteList) {
		...
		if (ignoreWhiteLists || ignoreWhiteList || whiteList.contains(command.toLowerCase())) {
			if (outputInfo)
	...
	private int aliasExecutor(Player player, Block block, String cmd, boolean ignorePerms) {

		if (!enableAlias) {
			return 0;
		}

		if (!(ignorePerms || player.hasPermission("buttoncommands.use.alias"))) {
		...
		for (String s : a.CMDS) {
			String[] strArr = Misc.concatCmd(s + " " + args);
			strArr[1] = Misc.insertAll(strArr[1], player, block);
			executor(player, block, strArr, a.IGPM, a.IGWL, true);
		}
